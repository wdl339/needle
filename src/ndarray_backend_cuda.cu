...
__device__ size_t idxToOffset(size_t index, CudaVec shape, CudaVec strides, size_t offset){
  size_t ret = offset;
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  if (gid >= size) return;
  size_t a_idx = idxToOffset(gid, shape, strides, offset);
  out[gid] = a[a_idx];
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid >= size) return;
  size_t out_idx = idxToOffset(gid, shape, strides, offset);
  out[out_idx] = a[gid];
}
...
__global__ void MatmulKernel(const float* A, const float* B, float* out, uint32_t M,
                              uint32_t N, uint32_t P) {
  int xbase = blockIdx.x * TILE;
  int ybase = blockIdx.y * TILE;
  int xthread = xbase + V * threadIdx.x;
  int ythread = ybase + V * threadIdx.y;
  ...
  for (int i = 0; i < V; ++i) {
    for (int j = 0; j < V; ++j) {
      int row_out = ythread + i;
      int col_out = xthread + j;
      if (row_out < M && col_out < P) {
        out[row_out * P + col_out] = c[i][j];
      }
    }
  }
}
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  dim3 block_dim = dim3(TILE / V, TILE / V, 1);
...
#define DEFINE_EWISW_OP(NAME, KERNEL) \
  m.def(NAME, [](const CudaArray& a, const CudaArray& b, CudaArray* out) { \
    CudaDims dim = CudaOneDim(out->size); \
    KERNEL<<<dim.grid, dim.block>>>(a.ptr, b.ptr, out->ptr, out->size); \
  });

#define DEFINE_SCALAR_OP(NAME, KERNEL) \
  m.def(NAME, [](const CudaArray& a, scalar_t val, CudaArray* out) { \
    CudaDims dim = CudaOneDim(out->size); \
    KERNEL<<<dim.grid, dim.block>>>(a.ptr, val, out->ptr, out->size); \
  });

#define DEFINE_SINGLE_OP(NAME, KERNEL) \
  m.def(NAME, [](const CudaArray& a, CudaArray* out) { \
    CudaDims dim = CudaOneDim(out->size); \
    KERNEL<<<dim.grid, dim.block>>>(a.ptr, out->ptr, out->size); \
  });
...
PYBIND11_MODULE(ndarray_backend_cuda, m) {
  ...
  DEFINE_EWISW_OP("ewise_mul", EwiseMulKernel);
  DEFINE_SCALAR_OP("scalar_mul", ScalarMulKernel);
  DEFINE_EWISW_OP("ewise_div", EwiseDivKernel);
  DEFINE_SCALAR_OP("scalar_div", ScalarDivKernel);
  DEFINE_SCALAR_OP("scalar_power", ScalarPowerKernel);
  DEFINE_EWISW_OP("ewise_maximum", EwiseMaximumKernel);
  DEFINE_SCALAR_OP("scalar_maximum", ScalarMaximumKernel);
  DEFINE_EWISW_OP("ewise_eq", EwiseEqKernel);
  DEFINE_SCALAR_OP("scalar_eq", ScalarEqKernel);
  DEFINE_EWISW_OP("ewise_ge", EwiseGeKernel);
  DEFINE_SCALAR_OP("scalar_ge", ScalarGeKernel);
  DEFINE_SINGLE_OP("ewise_log", EwiseLogKernel);
  DEFINE_SINGLE_OP("ewise_exp", EwiseExpKernel);
  DEFINE_SINGLE_OP("ewise_tanh", EwiseTanhKernel);
