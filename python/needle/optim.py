...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for p in self.params:
            ...
            self.m[p] = self.beta1 * self.m[p].data + (1 - self.beta1) * (p.grad.data + self.weight_decay * p.data)
            self.v[p] = self.beta2 * self.v[p].data + (1 - self.beta2) * (p.grad.data + self.weight_decay * p.data) ** 2
            m_hat = self.m[p].data / (1 - self.beta1 ** self.t)
            v_hat = self.v[p].data / (1 - self.beta2 ** self.t)
            p.data = p.data - self.lr * m_hat.data / (ndl.ops.power_scalar(v_hat.data, 0.5) + self.eps).data
        ### END YOUR SOLUTION
