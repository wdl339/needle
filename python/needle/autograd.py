...
class Tensor(Value):
    ...
    def make_const(data, requires_grad=False):
        ...
        if not isinstance(data, NDArray):
            data = Tensor._array_from_numpy(data, device=cpu(), dtype="float32")
        tensor._init(
            None,
            [],
            cached_data=data
    ...
    for node in reverse_topo_order:
        ...
        if len(node.inputs) > 0:
            ...
            for i, child in enumerate(node.inputs):
                node_to_output_grads_list.setdefault(child, [])
                node_to_output_grads_list[child].append(gradient[i])
    ### END YOUR SOLUTION
