...
from .Release import ndarray_backend_cpu
from .Release import ndarray_backend_cuda
...
class NDArray:
    ...
    def as_strided(self, shape, strides):
        ...
        assert len(shape) == len(strides), f"shape: {shape}, strides: {strides}"
        return NDArray.make(
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        assert all([self.shape[i] == new_shape[i] or self.shape[i] == 1 for i in range(len(self.shape))]), \
            f"shape: {self.shape}, new_shape: {new_shape}"
        new_strides = tuple([self.strides[i] if self.shape[i] == new_shape[i] else 0 for i in range(len(self.shape))])
    ...
    def __getitem__(self, idxs):
        ...
        offset = self._offset + reduce(operator.add, [s.start * self.strides[i] for i, s in enumerate(idxs)])
...
def array(a, dtype="float32", device=None):
    ...
    dtype = "float32" if dtype is None else dtype
    assert dtype == "float32", f"ture type: {dtype}"
